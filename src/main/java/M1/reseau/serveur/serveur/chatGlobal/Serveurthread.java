...
class Serveurthread implements Runnable
{
    ...
    private ServeurChatTCP _servchat; // pour utilisation des méthodes de la classe principale
    ...
    Serveurthread(Socket s, ServeurChatTCP serveur) // le param s est donnée dans BlablaServ par ss.accept()
    {
        _servchat=serveur; // passage de local en global (pour gestion dans les autres méthodes)
    ...
    public void run()
    {
            ...
            while(_in.read(charCur, 0, 1)!=-1) // attente en boucle des messages provenant du client (bloquant sur _in.read())
            {
                ...
                else if(!message.equalsIgnoreCase("")) // juste une vérification de principe
                {
                    if(charCur[0]=='\u0000') // le dernier caractère était '\u0000' (char de terminaison nulle)
                        ...
                        _servchat.sendAll(message,""+charCur[0]);
                    else _servchat.sendAll(message,""); // sinon on envoie le message à tous
        ...
        finally // finally se produira le plus souvent lors de la déconnexion du client
        {
            try
            {
                ...
                _servchat.delClient(_numClient); // on supprime le client de la liste
